---
name: deploy
description: "Deploy agent to Databricks Apps using DAB (Databricks Declarative Automation Bundles). Use when: (1) User says 'deploy', 'push to databricks', or 'bundle deploy', (2) 'App already exists' error occurs, (3) Need to bind/unbind existing apps, (4) Debugging deployed apps, (5) Querying deployed app endpoints."
---
...
databricks bundle run multiagent-app
```
...
The resource key `multiagent-app` matches the app name in `databricks.yml` under `resources.apps`.
...
databricks bundle deployment bind multiagent-app <existing-app-name> --auto-approve
```
...
```bash
databricks bundle deployment unbind multiagent-app
```
...
| App running old code after deploy | Run `databricks bundle run multiagent-app` after deploy |
