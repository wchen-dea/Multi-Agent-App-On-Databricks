...
2. Subclass it to override `transform_stream_event` (replaces Variable IDs in streamed events)
...
```python
_FAKE_ID_PREFIX = "resp_Variable_"


def replace_fake_id(obj, real_id: str):
    """Recursively replace any resp_Variable_* ID with real_id."""
    if isinstance(obj, dict):
        return {k: replace_fake_id(v, real_id) for k, v in obj.items()}
    elif isinstance(obj, list):
        return [replace_fake_id(item, real_id) for item in obj]
...
| Stream events have Variable IDs | Missing `transform_stream_event` | Ensure `AgentServer` subclass overrides it |
