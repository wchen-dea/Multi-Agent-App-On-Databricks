...
> **Note:** The `agent_server/agent.py` scaffold is intentionally framework-agnostic — it contains the `@invoke`/`@stream` decorator pattern with TODO Variables. Step 3 (Migrate the Agent Code) will replace these Variables with the actual agent logic from the original Model Serving endpoint.
...
| **Deploy to Databricks Apps** | Configure databricks.yml resources and deploy with Databricks Declarative Automation Bundles |
...
This step uses Databricks Declarative Automation Bundles (DAB) to deploy. The scaffold includes a `databricks.yml` that you need to update with the app name and resources from the original model.
...
### 6.3 Deploy with Databricks Declarative Automation Bundles
